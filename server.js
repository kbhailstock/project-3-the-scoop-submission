...
function createComment(url, request){
    ...
    //look if author X id has comment Id in array -- if not - push
    database.users[comment.username].commentIds.push(comment.id);
    //function to add newly created comment\'s id to the author\'s comment ID\'s'
      //find author\'s comment id\'s === database.author[commentId array]
...
function deleteComment(url, request){
  ...
  if (savedComment) {
    ...
    response.status = 204;
    //remove deleted comment from author's comment ids
         //pull author ids - find comment in that and delete (splice) 
    const userCommentIds = database.users[savedComment.username].commentIds;
      userCommentIds.splice(userCommentIds.indexOf(id), 1);
    //remove deleted comment from article's comment ids
